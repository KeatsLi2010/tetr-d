...
export function DgLabSettingsPanel({
  ...
  return <section className="panel dglab-panel" id="dglab">
    ...
    <div className="dglab-settings-grid">
      ...
      <Field label={`应用强度上限 ${config.maxStrength}`} help="应用硬上限 80；仍受设备软上限约束。"><input max={80} min={0} onChange={(event) => set("maxStrength", range(Number(event.currentTarget.value), 0, 80))} type="range" value={config.maxStrength} /></Field>
