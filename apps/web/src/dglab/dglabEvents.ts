...
function feedbackCount(value: number): number {
...
export function soloLockPenaltyEvents(
  ...
  if (previousBackToBack > 0 && lock.backToBack === 0) {
    events.push({ kind: "b2bBreak", amount: feedbackCount(previousBackToBack), source: "solo" });
  ...
  if (lock.combo >= 0 && lock.combo > previousCombo) {
    events.push({ kind: "combo", amount: feedbackCount(lock.combo + 1), source: "solo" });
  }
...
export function duelStatePenaltyEvents(
  ...
  const events: DgLabPenaltyEvent[] = [];
  if (previous.backToBack > 0 && next.backToBack === 0) events.push({ kind: "b2bBreak", amount: feedbackCount(previous.backToBack), source: "duel" });
  else if (next.backToBack > previous.backToBack && next.backToBack > 0) events.push({ kind: "b2bContinue", amount: 1, source: "duel" });
  if (next.combo > previous.combo && next.combo >= 0) events.push({ kind: "combo", amount: feedbackCount(next.combo + 1), source: "duel" });
